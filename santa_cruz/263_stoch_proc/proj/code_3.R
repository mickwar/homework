...
rm.leap = grep("-02-29", tt)
daily.10 = aggregate(y[-rm.leap], list(substr(tt[-rm.leap], 5, 10)), function(x) quantile(x, 0.93))
qq = daily.10$x
p = 6
X = make.X(1:365, p)
bhat = solve(t(X) %*% X) %*% t(X) %*% log(qq)
...
# pred.y = double(n)
# tmp.ind = grep("(-12-)|(-01-)|(-02-)", tt)
# pred.y[tmp.ind] = quantile(y[tmp.ind], 0.95)
# tmp.ind = grep("(-03-)|(-04-)|(-05-)", tt)
# pred.y[tmp.ind] = quantile(y[tmp.ind], 0.95)
# tmp.ind = grep("(-06-)|(-07-)|(-08-)", tt)
# pred.y[tmp.ind] = quantile(y[tmp.ind], 0.95)
# tmp.ind = grep("(-09-)|(-10-)|(-11-)", tt)
# pred.y[tmp.ind] = quantile(y[tmp.ind], 0.95)

plot(y, type='l', xlim = c(1, 2000))
plot(y, type='l', xlim = c(n-2000, n))
#lines(xx, qq, col = 'red')
lines(pred.x, pred.y, col = 'darkgreen', lwd = 3)
abline(v = 365*seq(0, 49), lty=2, col = 'gray')
#abline(v = seq(1, 365, length = 13), lty = 2, col = 'gray')


plot(y - pred.y, xlim = c(0, 2500))
abline(h = 0)
...
#for (i in 1:365)
#    zeta[i,] = 1/365*(1 + ksi_vec[i,]*(u.year[i] - mu_vec[i,])/sig_vec[i,])^(-1/ksi_vec[i,])
...
#       z[,i] =  u.year[i] + tmp_squig/ksi_vec[i,]*( (zeta[i,]*m/365)^(ksi_vec[i,]) - 1)
#       z[,i] =  mu_vec[i,] + sig_vec[i,]/ksi_vec[i,]*( (m/1)^(ksi_vec[i,]) - 1)
        }
    return (z)
    }
...
rm.leap = grep("-02-29", tt)
daily.10 = aggregate(y[-rm.leap], list(substr(tt[-rm.leap], 5, 10)), function(x) tail(sort(x), 10))

plot(z.mm, pch = 16, ylim = c(0, max(z.10, y)))
