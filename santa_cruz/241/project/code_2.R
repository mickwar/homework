...
#           }
    ...
    sumsq = double(n)
    for (i in 1:n)
        sumsq[i] = sum((dat[[i]]$xy[,2]-jc(dat[[i]]$xy[,1],
    ...
    for (j in 1:n.star){
        ...
        candsumsq = 0
        for (i in temp.w)
            candsumsq = candsumsq + sum((dat[[i]]$xy[,2]-jc(dat[[i]]$xy[,1],
                c(dat[[i]]$strain_rate, dat[[i]]$temperature), cand))^2)
        currsumsq = 0
        for (i in temp.w)
            currsumsq = currsumsq + sum((dat[[i]]$xy[,2]-jc(dat[[i]]$xy[,1],
                c(dat[[i]]$strain_rate, dat[[i]]$temperature), c.theta[i,]))^2)
